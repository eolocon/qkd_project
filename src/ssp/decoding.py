...
class Decoder:
	"""Decoder for Six State protocol.
...
class DecodingCircuit:
	"""Class to create qiskit circuit to perform measurements according to Six State protocol.
	
	The circuit is created to perform mesurements Bob's chosen basis (b_basis). 
	
	b_basis has values in {0,1, 2}, with the following meaning:

		0 -> Z basis
		1 -> X basis
		2 -> Y basis

	Since with qiskit is possible to measure only in the Z basis
	...
	In this case: 
	if b_basis = 1, the Hadamard operator is applied;
	if b_basis = 2, the adjoint of the S operator, followed by Hadamard operator are applied
